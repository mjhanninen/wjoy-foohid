...
{
    return [NotificationWindowView bestViewRectForTitle:[notification title]
                                                   text:[notification text]];
}

+ (NotificationWindow *)newWindowWithNotification:
                            (UserNotification *)notification
                                            frame:(NSRect)frame
{
    return
        [[[NotificationWindow alloc] initWithNotification:notification
...
- (id)initWithNotification:(UserNotification *)notification frame:(NSRect)frame
{
    ...
    NotificationWindowView *contentView =
        [[NotificationWindowView alloc] initWithFrame:contentViewFrame];

    [contentView
        setIcon:[[NSApplication sharedApplication] applicationIconImage]];
...
- (void)startAutocloseTimer:(NSTimeInterval)timeout
{
    if (m_AutocloseTimer == nil)
    {
        ...
        [[NSRunLoop currentRunLoop] addTimer:m_AutocloseTimer
