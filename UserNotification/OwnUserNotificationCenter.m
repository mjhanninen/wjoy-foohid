...
+ (void)load
{
    ...
    OwnUserNotificationCenter *center =
        [[OwnUserNotificationCenter alloc] init];
...
- (NSString *)name { return @"own"; }

- (NSUInteger)merit { return 2; }
...
{
    NSNumber *timeout =
        [NSNumber numberWithDouble:[[NotificationSystem sharedInstance]
                                       notificationTimeout]];
    NSNumber *screenCorner = [NSNumber
        numberWithInteger:[[NotificationSystem sharedInstance] screenCorner]];

    return [NSDictionary
        dictionaryWithObjectsAndKeys:timeout, UserNotificationCenterTimeoutKey,
                                     screenCorner,
...
{
    NSNumber *number =
        [preferences objectForKey:UserNotificationCenterTimeoutKey];
    if (number != nil)
    {
