//
//  RightTopNotificationLayoutManager.m
//  UserNotification
//
...
@implementation RightTopNotificationLayoutManager
                       ...
                       rect:(NSRect *)resultRect
{
    ...
    if ((result.origin.x > firstFrame.origin.x) ||
        (result.origin.y >=
         (firstFrame.origin.y + firstFrame.size.height + 10.0f)))
    {
        if (resultRect != NULL)
            *resultRect = result;
                   ...
                   rect:(NSRect *)resultRect
{
    ...
    {
        NSRect screenRect = [self screenRect];
        if ((firstRect.origin.y - screenRect.origin.y) >=
        ...
        if (lostSpace >= bestRect.size.height)
        {
            ...
            {
                *resultRect =
                    NSMakeRect(secondRect.origin.x,
    ...
    float lostSpace = firstRect.origin.y - secondRect.origin.y -
                      secondRect.size.height - 20.0f;
    if (lostSpace >= bestRect.size.height)
    {
        if (resultRect != NULL)
        {
                     ...
                     rect:(NSRect *)resultRect
{
    ...
    if ((lastRect.origin.y - screenRect.origin.y) >=
        (bestRect.size.height + 10.0f))
    {
        if (resultRect != NULL)
        {
    ...
    if (resultRect != NULL)
    {
        *resultRect = NSMakeRect(lastRect.origin.x,
                                 screenRect.origin.y + screenRect.size.height -
                                     bestRect.size.height,
                                 bestRect.size.width, bestRect.size.height);
    }
