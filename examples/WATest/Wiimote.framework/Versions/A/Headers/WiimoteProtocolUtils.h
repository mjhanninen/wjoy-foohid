...
#define WiimoteDeviceCheckStickCalibration(stickCalibration, minValue,         \
    ...
    {                                                                          \
        ...
        if ((stickCalibration).y.min == (maxValue))                            \
            (stickCalibration).y.min = (minValue);                             \
                                                                               \
        if ((stickCalibration).x.center == (minValue) ||                       \
            (stickCalibration).x.center == (maxValue))                         \
            (stickCalibration).x.center = (centerValue);                       \
                                                                               \
        if ((stickCalibration).y.center == (minValue) ||                       \
            (stickCalibration).y.center == (maxValue))                         \
...
#define WiimoteDeviceNormalizeStickCoordinateEx(value, min, center, max,       \
                                                result)                        \
    {                                                                          \
        ...
        if ((value) <= (center))                                               \
        {                                                                      \
            wiimote_device_norm_value_ =                                       \
                ((CGFloat)(value) - (CGFloat)(min)) /                          \
                    ((CGFloat)(center) - (CGFloat)(min)) -                     \
                1.0f;                                                          \
        }                                                                      \
        else                                                                   \
        {                                                                      \
            wiimote_device_norm_value_ =                                       \
                ((CGFloat)(value) - (CGFloat)(center)) /                       \
...
#define WiimoteDeviceNormalizeStick(pointX, pointY, stickCalibrationData,      \
                                    resultPoint)                               \
    {                                                                          \
        WiimoteDeviceNormalizeStickCoordinateEx(                               \
            (pointX), (stickCalibrationData).x.min,                            \
            (stickCalibrationData).x.center, (stickCalibrationData).x.max,     \
            (resultPoint).x);                                                  \
                                                                               \
        WiimoteDeviceNormalizeStickCoordinateEx(                               \
            (pointY), (stickCalibrationData).y.min,                            \
            (stickCalibrationData).y.center, (stickCalibrationData).y.max,     \
            (resultPoint).y);                                                  \
    }

#define WiimoteDeviceNormilizeShift(shiftValue, min, center, max, result)      \
    WiimoteDeviceNormalizeStickCoordinateEx((shiftValue), (min), (center),     \
                                            (max), (result));                  \
