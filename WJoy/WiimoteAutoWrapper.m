...
+ (NSUInteger)maxConnectedDevices { return maxConnectedDevices; }
...
+ (void)setMaxConnectedDevices:(NSUInteger)count
{
    ...
    while ([[Wiimote connectedDevices] count] > count)
    {
        ...
        Wiimote *device =
            [connectedDevices objectAtIndex:[connectedDevices count] - 1];
...
- (void)wiimoteDisconnected:(Wiimote *)wiimote { [self release]; }
      ...
      positionChanged:(CGFloat)position
{
    ...
    [m_HIDState setPointer:WiimoteClassicControllerStickCount
                  position:m_ShiftsState];
}
    ...
    positionChanged:(NSPoint)position
{
    ...
    NSLog(@"\nMinLx: %f\tMinLy: %f\tMaxLx: %f\tMaxLy: %f", minL.x, minL.y,
          maxL.x, maxL.y);
    NSLog(@"\nMinRx: %f\tMinRy: %f\tMaxRx: %f\tMaxRy: %f", minR.x, minR.y,
          maxR.x, maxR.y);
