...
+ (NSUInteger)maxButtonCount { return [VHIDButtonCollection maxButtonCount]; }
        ...
        isRelative:(BOOL)isRelative
{
    ...
    m_Pointers =
        [[VHIDPointerCollection alloc] initWithPointerCount:pointerCount
                                                 isRelative:isRelative];

    m_State =
        [[NSMutableData alloc] initWithLength:[[m_Buttons state] length] +
...
- (VHIDDeviceType)type { return m_Type; }
...
- (NSUInteger)buttonCount { return [m_Buttons buttonCount]; }

- (NSUInteger)pointerCount { return [m_Pointers pointerCount]; }
...
- (NSData *)descriptor { return [[m_Descriptor retain] autorelease]; }
...
- (NSData *)state
{
    ...
    if (pointerState != nil)
    {
        memcpy(data + [buttonState length], [pointerState bytes],
               [pointerState length]);
    }
...
- (id<VHIDDeviceDelegate>)delegate { return m_Delegate; }

- (void)setDelegate:(id<VHIDDeviceDelegate>)obj { m_Delegate = obj; }
...
- (NSData *)createDescriptor
{
    ...
    NSMutableData *result = [NSMutableData
        dataWithLength:[buttonsHID length] + [pointersHID length] +
                       ((isMouse) ? (HIDDescriptorMouseAdditionalBytes)
