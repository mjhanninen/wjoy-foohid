...
+ (unsigned char)translatePointerCoordinateIndex:
    (NSUInteger)pointerCoordinateIndex
{
                             ...
                             stateSize:(NSUInteger *)stateSize
{
    ...
    *data = 0x01;
    ...
    {
        coordinateIndex =
            [VHIDPointerCollection translatePointerCoordinateIndex:i];
        *data = 0x09;
        data++;
...
- (id)initWithPointerCount:(NSUInteger)pointerCount isRelative:(BOOL)isRelative
{
    ...
    m_Descriptor =
        [[VHIDPointerCollection descriptorWithPointerCount:pointerCount
...
- (NSUInteger)pointerCount { return m_PointerCount; }
...
- (NSPoint)pointerPosition:(NSUInteger)pointerIndex
{
    ...
    char *data =
        (char *)[m_State mutableBytes] + pointerIndex * HIDStatePointerSize;

    return NSMakePoint([VHIDPointerCollection clipCoordinateFrom:*data],
...
- (void)reset { memset([m_State mutableBytes], 0, [m_State length]); }

- (NSData *)descriptor { return [[m_Descriptor retain] autorelease]; }

- (NSData *)state { return [[m_State retain] autorelease]; }
