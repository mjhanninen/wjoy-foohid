...
static const unsigned char buttonMasks[] = {1, 2, 4, 8, 16, 32, 64, 128};
                            ...
                            stateSize:(NSUInteger *)stateSize
{
    ...
    NSMutableData *result =
        [NSMutableData dataWithCapacity:HIDDescriptorSizeWithPadding];

    if (stateSize != NULL)
        *stateSize = (buttonCount + paddingBits) / 8;

    [result setLength:((paddingBits == 0) ? (HIDDescriptorSizeWithoutPadding)
...
+ (NSUInteger)maxButtonCount { return 255; }
...
- (id)initWithButtonCount:(NSUInteger)buttonCount
{
    ...
    m_Descriptor =
        [[VHIDButtonCollection descriptorWithButtonCount:buttonCount
...
- (NSUInteger)buttonCount { return m_ButtonCount; }
