...
{
    NSLog(@"%@: %s (%llu) [%s]: %@", [OCLog levelAsString:level], sourceFile,
          (unsigned long long)line, functionName, message);
}
...
- (OCLogLevel)level { return m_Level; }

- (void)setLevel:(OCLogLevel)level { m_Level = level; }

- (NSObject<OCLogHandler> *)handler { return [[m_Handler retain] autorelease]; }
