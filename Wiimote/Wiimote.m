...
NSString *WiimoteBeginDiscoveryNotification =
    @"WiimoteBeginDiscoveryNotification";
NSString *WiimoteEndDiscoveryNotification = @"WiimoteEndDiscoveryNotification";

NSString *WiimoteUseOneButtonClickConnectionChangedNotification =
    @"WiimoteUseOneButtonClickConnectionChangedNotification";

NSString *WiimoteUseOneButtonClickConnectionKey =
    @"WiimoteUseOneButtonClickConnectionKey";

@implementation Wiimote

+ (BOOL)isBluetoothEnabled { return [WiimoteInquiry isBluetoothEnabled]; }
...
+ (void)setUseOneButtonClickConnection:(BOOL)useOneButtonClickConnection
{
    ...
    NSDictionary *userInfo = [NSDictionary
        dictionaryWithObject:[NSNumber
                                 numberWithBool:useOneButtonClickConnection]
                      forKey:WiimoteUseOneButtonClickConnectionKey];

    [[WiimoteInquiry sharedInquiry]
        setUseOneButtonClickConnection:useOneButtonClickConnection];
...
+ (BOOL)isDiscovering { return [[WiimoteInquiry sharedInquiry] isStarted]; }
...
+ (NSArray *)connectedDevices { return [Wiimote connectedWiimotes]; }

- (BOOL)isConnected { return [m_Device isConnected]; }

- (void)disconnect { [m_Device disconnect]; }
...
- (NSData *)address { return [m_Device address]; }

- (NSString *)addressString { return [m_Device addressString]; }

- (NSString *)modelName { return [[m_ModelName retain] autorelease]; }

- (void)playConnectEffect
{
    [self setVibrationEnabled:YES];
    [[NSRunLoop currentRunLoop]
        runUntilDate:[NSDate dateWithTimeIntervalSinceNow:0.35]];
    [self setVibrationEnabled:NO];
}

- (NSUInteger)highlightedLEDMask { return [m_LEDPart highlightedLEDMask]; }
...
- (BOOL)isVibrationEnabled { return [m_VibrationPart isVibrationEnabled]; }
...
- (CGFloat)batteryLevel { return [m_BatteryPart batteryLevel]; }

- (BOOL)isBatteryLevelLow { return [m_BatteryPart isBatteryLevelLow]; }

- (BOOL)isIREnabled { return [m_IRPart isEnabled]; }

- (void)setIREnabled:(BOOL)enabled { [m_IRPart setEnabled:enabled]; }

- (WiimoteIRPoint *)irPoint:(NSUInteger)index { return [m_IRPart point:index]; }
...
- (void)detectMotionPlus { [m_ExtensionPart detectMotionPlus]; }

- (void)reconnectExtension { [m_ExtensionPart reconnectExtension]; }

- (void)disconnectExtension { [m_ExtensionPart disconnectExtension]; }

- (void)requestUpdateState { [m_Device requestStateReport]; }
...
- (NSDictionary *)userInfo { return [[m_UserInfo retain] autorelease]; }
...
- (id)delegate { return [[m_PartSet eventDispatcher] delegate]; }
