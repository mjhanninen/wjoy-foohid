...
+ (void)load { [WiimotePart registerPartClass:[WiimoteIRPart class]]; }
          ...
          ioManager:(WiimoteIOManager *)ioManager
{
    ...
    m_ReportType = -1;
    m_ReportCounter = 0;
    m_Points = [[NSArray alloc]
        initWithObjects:[WiimoteIRPoint pointWithOwner:owner index:0],
                        [WiimoteIRPoint pointWithOwner:owner index:1],
                        [WiimoteIRPoint pointWithOwner:owner index:2],
                        [WiimoteIRPoint pointWithOwner:owner index:3], nil];
...
- (NSSet *)allowedReportTypeSet
{
    ...
    {
        allowedReportTypeSet = [[NSSet alloc]
            initWithObjects:
                [NSNumber
                    numberWithInteger:
...
- (void)handleReport:(WiimoteDeviceReport *)report
{
    ...
    switch (reportType)
    {
        case WiimoteDeviceReportTypeButtonAndAccelerometerAndIR12BytesState:
            irDataOffset = sizeof(WiimoteDeviceButtonState) +
                           WiimoteDeviceAccelerometerDataSize;
            irDataSize = 12;
            break;
    ...
    [self handleIRData:[report data] + irDataOffset length:irDataSize];
}
...
- (void)enableHardware:(WiimoteDeviceIRMode)irMode
{
    static const uint8_t sensitivityBlock1[] = {0x00, 0x00, 0x00, 0x00, 0x00,
                                                0x00, 0x90, 0x00, 0x41};
    ...
    [[self ioManager] postCommand:WiimoteDeviceCommandTypeSetIREnabledState
                             data:&data
                           length:sizeof(data)];
    usleep(50000);

    [[self ioManager] postCommand:WiimoteDeviceCommandTypeSetIREnabledState2
                             data:&data
                           length:sizeof(data)];
    usleep(50000);

    data = WiimoteDeviceIRBeginInitValue;
    [[self ioManager] writeMemory:WiimoteDeviceIRInitAddress
                             data:&data
                           length:sizeof(data)];
    usleep(50000);

    [[self ioManager] writeMemory:WiimoteDeviceIRSensitivityBlockAddress1
                             data:sensitivityBlock1
                           length:sizeof(sensitivityBlock1)];
    usleep(50000);

    [[self ioManager] writeMemory:WiimoteDeviceIRSensitivityBlockAddress2
                             data:sensitivityBlock2
                           length:sizeof(sensitivityBlock2)];
    usleep(50000);
    ...
    data = irMode;
    [[self ioManager] writeMemory:WiimoteDeviceIRModeAddress
                             data:&data
                           length:sizeof(data)];
    usleep(50000);
