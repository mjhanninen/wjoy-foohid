...
{
    [[self delegate] wiimote:[self owner] motionPlus:motionPlus report:report];

    if ([self isStateNotificationsEnabled])
    {
        [self
            postNotification:WiimoteMotionPlusReportNotification
                       param:[NSData
                                 dataWithBytes:report
                                        length:sizeof(WiimoteMotionPlusReport)]
