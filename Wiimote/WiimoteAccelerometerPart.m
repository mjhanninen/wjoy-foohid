...
- (void)handleReport:(WiimoteDeviceReport *)report
{
    ...
    if ([report length] <
        sizeof(WiimoteDeviceButtonAndAccelerometerStateReport))
        return;
    ...
    uint16_t x = (((uint16_t)stateReport->accelerometerX) << 2) |
                 (((stateReport->accelerometerAdditionalX >> 5) & 0x3) << 0);
    uint16_t y = (((uint16_t)stateReport->accelerometerY) << 2) |
                 (((stateReport->accelerometerAdditionalYZ >> 5) & 0x1) << 1);
    uint16_t z = (((uint16_t)stateReport->accelerometerZ) << 2) |
                 (((stateReport->accelerometerAdditionalYZ >> 6) & 0x1) << 1);

    [m_Accelerometer setHardwareValueX:x y:y z:z];
}

- (void)disconnected { [m_Accelerometer reset]; }
...
{
    NSRange memRange =
        NSMakeRange(WiimoteDeviceCalibrationDataAddress,
...
- (void)wiimoteAccelerometer:(WiimoteAccelerometer *)accelerometer
             gravityChangedX:(CGFloat)x
                           ...
                           z:(CGFloat)z
{
    [[self eventDispatcher] postAccelerometerGravityChangedNotificationX:x
                                                                       y:y
                                                                       z:z];
}
                        ...
                        roll:(CGFloat)roll
{
    [[self eventDispatcher]
        postAccelerometerAnglesChangedNotificationPitch:pitch
                                                   roll:roll];
}
