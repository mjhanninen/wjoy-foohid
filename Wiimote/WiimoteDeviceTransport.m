...
- (NSString *)name { return [m_Device name]; }
...
- (NSData *)address
{
    ...
    {
        NSScanner *scanner =
            [[NSScanner alloc] initWithString:[components objectAtIndex:i]];
...
- (NSString *)addressString { return [m_Device address]; }

- (id)lowLevelDevice { return [[m_Device retain] autorelease]; }

- (BOOL)isOpen { return [m_Device isValid]; }

- (BOOL)open { return [m_Device setOptions:kIOHIDOptionsTypeSeizeDevice]; }
...
{
    [[self delegate] wiimoteDeviceTransport:self
...
- (NSString *)name { return [m_Device getName]; }
...
- (NSData *)address
{
    ...
    return [NSData dataWithBytes:address->data length:sizeof(address->data)];
}

- (NSString *)addressString { return [m_Device getAddressString]; }
...
- (BOOL)postBytes:(const uint8_t *)bytes length:(NSUInteger)length
{
    ...
    return ([m_DataChannel writeSync:buffer length:sizeof(buffer)] ==
            kIOReturnSuccess);
}
...
{
    return [[[WiimoteBluetoothDeviceTransport alloc]
        initWithBluetoothDevice:device] autorelease];
}

- (NSString *)name { return nil; }

- (NSData *)address { return nil; }

- (NSString *)addressString { return nil; }

- (id)lowLevelDevice { return nil; }
