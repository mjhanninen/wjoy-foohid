...
extern Boolean
IOBluetoothLocalDeviceAvailable(void);
extern IOReturn
IOBluetoothLocalDeviceGetPowerState(BluetoothHCIPowerState *state);
...
- (BOOL)isStarted { return (m_Inquiry != nil); }
...
- (void)hidManagerDeviceConnectedNotification:(NSNotification *)notification
{
    ...
    HIDDevice *device =
        [[notification userInfo] objectForKey:HIDManagerDeviceKey];
...
- (BOOL)isHIDDeviceAlreadyConnected:(HIDDevice *)device
                           wiimotes:(NSArray *)wiimotes
{
...
{
    NSEnumerator *en =
        [[[HIDManager manager] connectedDevices] objectEnumerator];
