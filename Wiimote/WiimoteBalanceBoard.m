...
{
    return NSMakeRange(WiimoteBalanceBoardCalibrationDataAddress,
...
+ (NSUInteger)minReportDataSize { return sizeof(WiimoteBalanceBoardReport); }
...
- (NSString *)name { return @"Balance Board"; }

- (double)topLeftPress { return m_TopLeftPress; }

- (double)topRightPress { return m_TopRightPress; }

- (double)bottomLeftPress { return m_BottomLeftPress; }

- (double)bottomRightPress { return m_BottomRightPress; }
            ...
            bottomRight:(double)bottomRight
{
    ...
    [[self eventDispatcher] postBalanceBoard:self
...
- (void)preprocessReport:(WiimoteBalanceBoardReport *)report
{
    ...
    report->bottomLeftPress =
        OSSwapBigToHostConstInt16(report->bottomLeftPress);
    report->bottomRightPress =
        OSSwapBigToHostConstInt16(report->bottomRightPress);
}

- (double)processPressValue:(double)value
                        kg0:(double)zero
                       kg17:(double)kg17
                       kg34:(double)kg34
{
...
- (void)handleReport:(const uint8_t *)extensionData length:(NSUInteger)length
{
    ...
    double bottomLeft =
        [self processPressValue:report.bottomLeftPress
    ...
    double bottomRight =
        [self processPressValue:report.bottomRightPress
