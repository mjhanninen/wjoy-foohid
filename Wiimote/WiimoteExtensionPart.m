...
- (const uint8_t *)extractExtensionReportPart:(WiimoteDeviceReport *)report
...
static NSInteger
sortExtensionClassesByMeritFn(Class cls1, Class cls2, void *context)
{
...
+ (void)load { [WiimotePart registerPartClass:[WiimoteExtensionPart class]]; }
...
- (void)disconnected { [self extensionDisconnected]; }
                                       ...
                                       length:(NSUInteger *)length;
{
    ...
    switch (reportType)
    {
        ...
        case WiimoteDeviceReportTypeButtonAndAccelerometerAndIR10BytesAndExtension6Bytes:
            extensionBytesOffset = sizeof(WiimoteDeviceButtonState) +
                                   WiimoteDeviceAccelerometerDataSize + 10;
...
- (void)processExtensionReport:(WiimoteDeviceReport *)report
{
    ...
    NSUInteger length = 0;
    const uint8_t *extensionReportPart =
        [self extractExtensionReportPart:report length:&length];
...
- (void)motionPlusDetectFinish:(NSNumber *)detected
{
    ...
    [WiimoteMotionPlusDetector activateMotionPlus:[self ioManager]
