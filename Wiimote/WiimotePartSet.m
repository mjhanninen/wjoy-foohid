...
- (id)initWithOwner:(Wiimote *)owner device:(WiimoteDevice *)device
{
    ...
    m_PartDictionary =
        [[NSMutableDictionary alloc] initWithCapacity:countPartClasses];
...
- (WiimoteDevice *)device { return m_Device; }
...
- (WiimoteDeviceReportType)bestReportType
{
    ...
    NSMutableSet *reportTypes = [NSMutableSet
        setWithObjects:
    ...
    {
        NSSet *partReports =
            [[m_PartArray objectAtIndex:i] allowedReportTypeSet];
    ...
    if ([reportTypes count] == 0 ||
        [reportTypes
            containsObject:[NSNumber numberWithInteger:
                                         WiimoteDeviceReportTypeButtonState]])
    {
        return WiimoteDeviceReportTypeButtonState;
    }

    return ((WiimoteDeviceReportType)[[reportTypes anyObject] integerValue]);
}
