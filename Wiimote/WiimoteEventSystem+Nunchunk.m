...
{
    static NSString *result[] = { @"Button.C", @"Button.Z" };

    WiimoteNunchuckButtonType type =
        [[userInfo objectForKey:WiimoteNunchuckButtonKey] integerValue];
...
{
    [self
        postEventForWiimoteExtension:[notification object]
                                path:[self pathForNunchuckButton:[notification
                                                                     userInfo]]
...
- (void)wiimoteNunchuckStickPositionChangedNotification:
    (NSNotification *)notification
{
    NSPoint position = [[[notification userInfo]
        objectForKey:WiimoteNunchuckStickPositionKey] pointValue];
...
- (void)wiimoteNunchuckAccelerometerAnglesChangedNotification:
    (NSNotification *)notification
{
    CGFloat pitch = [[[notification userInfo]
        objectForKey:WiimoteNunchuckAccelerometerPitchKey] doubleValue];
    CGFloat roll = [[[notification userInfo]
        objectForKey:WiimoteNunchuckAccelerometerRollKey] doubleValue];

    [self postEventForWiimoteExtension:[notification object]
                                  path:@"Accelerometer.Pitch"
                                 value:pitch];
    [self postEventForWiimoteExtension:[notification object]
                                  path:@"Accelerometer.Roll"
                                 value:roll];
}
